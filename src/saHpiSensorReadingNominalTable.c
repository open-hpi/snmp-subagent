...
oid saHpiSensorReadingNominalTable_oid[] =
  { saHpiSensorReadingNominalTable_TABLE_OID };
size_t saHpiSensorReadingNominalTable_oid_len =
OID_LENGTH (saHpiSensorReadingNominalTable_oid);
			   ...
			   SaHpiSensorNumT sensor_num)
{
  ...
  DEBUGMSGTL ((AGENT, "delete_ReadingNominal_row (%d, %d, %d). Entry.\n",
  ...
  if (ctx)
    {
      ...
      saHpiSensorReadingNominalTable_delete_row (ctx);
      rc = AGENT_ERR_NOERROR;
    }
  DEBUGMSGTL ((AGENT, "delete_ReadingNominal_row. Exit (rc: %d).\n", rc));
  return rc;
}


int
populate_ReadingNominal (SaHpiDomainIdT domain_id,
			 SaHpiResourceIdT resource_id,
			 ...
			 SaHpiSensorReadingT * reading)
{
  ...
  DEBUGMSGT ((AGENT, "\t--- populate_ReadingNominal, Entry,\n"));
  ...
  if (reading)
      ...
      if (!ctx)
	{
	   ...
	   */
	  ctx =
	    saHpiSensorReadingNominalTable_create_row (&sensor_reading_index);
	}
      if (!ctx)
	{
	  snmp_log (LOG_ERR,
		    "Not enough memory for a sensor ReadingNominal row!");
	  return AGENT_ERR_INTERNAL_ERROR;
	}

      if (modify_saHpiSensorReadingNominalTable_row (domain_id,
						     resource_id,
						     sensor_id,
						     sensor_category,
						     reading,
  ...
  DEBUGMSGT ((AGENT, "\t--- populate_ReadingNominal, Exit (rc: %d),\n", rc));
 ...
 */
static int
  saHpiSensorReadingNominalTable_row_copy
  (saHpiSensorReadingNominalTable_context * dst,
   saHpiSensorReadingNominalTable_context * src)
{
   ...
   */
  dst->saHpiSensorReadingNominalValuesPresent =
    src->saHpiSensorReadingNominalValuesPresent;

  dst->saHpiSensorReadingNominalRaw = src->saHpiSensorReadingNominalRaw;

  memcpy (dst->saHpiSensorReadingNominalInterpreted,
	  src->saHpiSensorReadingNominalInterpreted,
	  src->saHpiSensorReadingNominalInterpreted_len);
  dst->saHpiSensorReadingNominalInterpreted_len =
    src->saHpiSensorReadingNominalInterpreted_len;

  dst->saHpiSensorReadingNominalStatus = src->saHpiSensorReadingNominalStatus;

  memcpy (dst->saHpiSensorReadingNominalEventStatus,
	  src->saHpiSensorReadingNominalEventStatus,
	  src->saHpiSensorReadingNominalEventStatus_len);
  dst->saHpiSensorReadingNominalEventStatus_len =
    src->saHpiSensorReadingNominalEventStatus_len;
 ...
 */
netsnmp_index *saHpiSensorReadingNominalTable_delete_row
  (saHpiSensorReadingNominalTable_context * ctx)
{
  /*
 ...
 */
void
saHpiSensorReadingNominalTable_set_reserve1 (netsnmp_request_group * rg)
{

}

void
saHpiSensorReadingNominalTable_set_reserve2 (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNominalTable_set_action (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNominalTable_set_commit (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNominalTable_set_free (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNominalTable_set_undo (netsnmp_request_group * rg)
{
...
initialize_table_saHpiSensorReadingNominalTable (void)
{
  ...
  if (my_handler)
    {
      snmp_log (LOG_ERR,
		"initialize_table_saHpiSensorReadingNominalTable_handler called again\n");
      return;
    }
  ...
  my_handler =
    ...
    ("saHpiSensorReadingNominalTable",
     ...
     saHpiSensorReadingNominalTable_oid_len, HANDLER_CAN_RONLY);
  ...
  cb.container =
    netsnmp_container_find ("saHpiSensorReadingNominalTable_primary:"
			    "saHpiSensorReadingNominalTable:"
			    "table_container");
  ...
  cb.duplicate_row =
    (UserRowMethod *) saHpiSensorReadingNominalTable_duplicate_row;
  cb.delete_row = (UserRowMethod *) saHpiSensorReadingNominalTable_delete_row;
  cb.row_copy =
    (Netsnmp_User_Row_Operation *) saHpiSensorReadingNominalTable_row_copy;

  cb.can_delete =
    (Netsnmp_User_Row_Action *) saHpiSensorReadingNominalTable_can_delete;
 ...
 */
int
saHpiSensorReadingNominalTable_get_value (netsnmp_request_info * request,
					  netsnmp_index * item,
					  netsnmp_table_request_info
					  * table_info)
{
  netsnmp_variable_list *var = request->requestvb;
  saHpiSensorReadingNominalTable_context *context =
    (saHpiSensorReadingNominalTable_context *) item;

  switch (table_info->colnum)
    {

    case COLUMN_SAHPISENSORREADINGNOMINALVALUESPRESENT:
	    /** INTEGER = ASN_INTEGER */
      snmp_set_var_typed_value (var, ASN_INTEGER,
				(char *) &context->
				saHpiSensorReadingNominalValuesPresent,
				sizeof (context->
					saHpiSensorReadingNominalValuesPresent));
      break;

    case COLUMN_SAHPISENSORREADINGNOMINALRAW:
	    /** UNSIGNED32 = ASN_UNSIGNED */
      snmp_set_var_typed_value (var, ASN_UNSIGNED,
				(char *) &context->
				saHpiSensorReadingNominalRaw,
				sizeof (context->
					saHpiSensorReadingNominalRaw));
      break;

    case COLUMN_SAHPISENSORREADINGNOMINALINTERPRETED:
	    /** OCTETSTR = ASN_OCTET_STR */
      snmp_set_var_typed_value (var, ASN_OCTET_STR,
				(char *) &context->
				saHpiSensorReadingNominalInterpreted,
				context->
				saHpiSensorReadingNominalInterpreted_len);
      break;

    case COLUMN_SAHPISENSORREADINGNOMINALSTATUS:
	    /** INTEGER = ASN_INTEGER */
      snmp_set_var_typed_value (var, ASN_INTEGER,
				(char *) &context->
				saHpiSensorReadingNominalStatus,
				sizeof (context->
					saHpiSensorReadingNominalStatus));
      break;

    case COLUMN_SAHPISENSORREADINGNOMINALEVENTSTATUS:
	    /** OCTETSTR = ASN_OCTET_STR */
      snmp_set_var_typed_value (var, ASN_OCTET_STR,
				(char *) &context->
				saHpiSensorReadingNominalEventStatus,
				context->
				saHpiSensorReadingNominalEventStatus_len);
      break;
