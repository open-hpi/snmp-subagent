...
oid saHpiSensorReadingNormalMinTable_oid[] =
  { saHpiSensorReadingNormalMinTable_TABLE_OID };
size_t saHpiSensorReadingNormalMinTable_oid_len =
OID_LENGTH (saHpiSensorReadingNormalMinTable_oid);

static int
modify_saHpiSensorReadingNormalMinTable_row (SaHpiDomainIdT domain_id,
			   ...
			   SaHpiSensorNumT sensor_num)
{
  ...
  DEBUGMSGTL ((AGENT, "delete_ReadingNormalMin_row (%d, %d, %d). Entry.\n",
  ...
  if (ctx)
    {
      ...
      saHpiSensorReadingNormalMinTable_delete_row (ctx);
      rc = AGENT_ERR_NOERROR;
    }
  DEBUGMSGTL ((AGENT, "delete_ReadingNormalMin_row. Exit (rc: %d).\n", rc));
  return rc;
}


int
populate_ReadingNormalMin (SaHpiDomainIdT domain_id,
			 SaHpiResourceIdT resource_id,
			 ...
			 SaHpiSensorReadingT * reading)
{
  ...
  DEBUGMSGT ((AGENT, "\t--- populate_ReadingNormalMin, Entry,\n"));
  ...
  if (reading)
      ...
      if (!ctx)
	{
	   ...
	   */
	  ctx =
	    saHpiSensorReadingNormalMinTable_create_row (&sensor_reading_index);
	}
      if (!ctx)
	{
	  snmp_log (LOG_ERR,
		    "Not enough memory for a sensor ReadingNormalMin row!");
	  return AGENT_ERR_INTERNAL_ERROR;
	}

      if (modify_saHpiSensorReadingNormalMinTable_row (domain_id,
						     resource_id,
						     sensor_id,
						     sensor_category,
						     reading,
  ...
  DEBUGMSGT ((AGENT, "\t--- populate_ReadingNormalMin, Exit (rc: %d),\n", rc));
					   ...
					   * ctx)
{
  ...
  DEBUGMSGTL ((AGENT, "Modify saHpiSensorReadingNormalMinTable_ctx: Entry.\n"));
  ...
  if (ctx)
      ...
      if (ctx->hash != 0)
	{
	  ...
	  if ((ctx->domain_id == domain_id) &&
	      ...
	      (ctx->sensor_id == sensor_num))
	    {
	      ...
	      DEBUGMSGTL ((AGENT,
			   "Updating ReadingNormalMinTable row [%d, %d, %d]\n",
			   domain_id, resource_id, sensor_num));
	    }
	}
	...
	hash = 1;		/* Need this - we consider hash
      ...
      build_reading_strings (reading,
			     sensor_category,
			     &ctx->saHpiSensorReadingNormalMinValuesPresent,
			     &ctx->saHpiSensorReadingNormalMinRaw,
			     ctx->saHpiSensorReadingNormalMinInterpreted,
			     &ctx->saHpiSensorReadingNormalMinInterpreted_len,
			     SENSOR_READING_INTER_MAX,
			     &ctx->saHpiSensorReadingNormalMinStatus,
			     ctx->saHpiSensorReadingNormalMinEventStatus,
			     &ctx->saHpiSensorReadingNormalMinEventStatus_len,
			     SENSOR_READING_EVENT_MAX);

      /* END */
      DEBUGMSGTL ((AGENT, "Modify saHpiSensorReadingNormalMinTable_ctx: Exit"));
  ...
  DEBUGMSGTL ((AGENT,
	       "Modify saHpiSensorReadingNormalMinTable_ctx: Exit (NULL DATA)"));
 ...
 */
static int
  saHpiSensorReadingNormalMinTable_row_copy
  (saHpiSensorReadingNormalMinTable_context * dst,
   saHpiSensorReadingNormalMinTable_context * src)
{
   ...
   */
  dst->saHpiSensorReadingNormalMinValuesPresent =
    src->saHpiSensorReadingNormalMinValuesPresent;

  dst->saHpiSensorReadingNormalMinRaw = src->saHpiSensorReadingNormalMinRaw;

  memcpy (dst->saHpiSensorReadingNormalMinInterpreted,
	  src->saHpiSensorReadingNormalMinInterpreted,
	  src->saHpiSensorReadingNormalMinInterpreted_len);
  dst->saHpiSensorReadingNormalMinInterpreted_len =
    src->saHpiSensorReadingNormalMinInterpreted_len;

  dst->saHpiSensorReadingNormalMinStatus = src->saHpiSensorReadingNormalMinStatus;

  memcpy (dst->saHpiSensorReadingNormalMinEventStatus,
	  src->saHpiSensorReadingNormalMinEventStatus,
 ...
 */
netsnmp_index *saHpiSensorReadingNormalMinTable_delete_row
  (saHpiSensorReadingNormalMinTable_context * ctx)
{
  /*
 ...
 */
void
saHpiSensorReadingNormalMinTable_set_reserve1 (netsnmp_request_group * rg)
{

}

void
saHpiSensorReadingNormalMinTable_set_reserve2 (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNormalMinTable_set_action (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNormalMinTable_set_commit (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNormalMinTable_set_free (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorReadingNormalMinTable_set_undo (netsnmp_request_group * rg)
{
...
initialize_table_saHpiSensorReadingNormalMinTable (void)
{
  ...
  if (my_handler)
    {
      snmp_log (LOG_ERR,
		"initialize_table_saHpiSensorReadingNormalMinTable_handler called again\n");
      return;
    }
  ...
  my_handler =
    ...
    ("saHpiSensorReadingNormalMinTable",
     ...
     saHpiSensorReadingNormalMinTable_oid_len, HANDLER_CAN_RONLY);
  ...
  cb.container =
    netsnmp_container_find ("saHpiSensorReadingNormalMinTable_primary:"
			    "saHpiSensorReadingNormalMinTable:"
			    "table_container");
  ...
  cb.duplicate_row =
    (UserRowMethod *) saHpiSensorReadingNormalMinTable_duplicate_row;
  cb.delete_row = (UserRowMethod *) saHpiSensorReadingNormalMinTable_delete_row;
  cb.row_copy =
    (Netsnmp_User_Row_Operation *) saHpiSensorReadingNormalMinTable_row_copy;

  cb.can_delete =
    (Netsnmp_User_Row_Action *) saHpiSensorReadingNormalMinTable_can_delete;
 ...
 */
int
saHpiSensorReadingNormalMinTable_get_value (netsnmp_request_info * request,
					  netsnmp_index * item,
					  netsnmp_table_request_info
					  * table_info)
{
  netsnmp_variable_list *var = request->requestvb;
  saHpiSensorReadingNormalMinTable_context *context =
    (saHpiSensorReadingNormalMinTable_context *) item;

  switch (table_info->colnum)
    {

    case COLUMN_SAHPISENSORREADINGNORMALMINVALUESPRESENT:
	    /** INTEGER = ASN_INTEGER */
      snmp_set_var_typed_value (var, ASN_INTEGER,
				(char *) &context->
				saHpiSensorReadingNormalMinValuesPresent,
				sizeof (context->
					saHpiSensorReadingNormalMinValuesPresent));
      break;

    case COLUMN_SAHPISENSORREADINGNORMALMINRAW:
	    /** UNSIGNED32 = ASN_UNSIGNED */
      snmp_set_var_typed_value (var, ASN_UNSIGNED,
				(char *) &context->
				saHpiSensorReadingNormalMinRaw,
				sizeof (context->
					saHpiSensorReadingNormalMinRaw));
      break;

    case COLUMN_SAHPISENSORREADINGNORMALMININTERPRETED:
	    /** OCTETSTR = ASN_OCTET_STR */
      snmp_set_var_typed_value (var, ASN_OCTET_STR,
				(char *) &context->
				saHpiSensorReadingNormalMinInterpreted,
				context->
				saHpiSensorReadingNormalMinInterpreted_len);
      break;

    case COLUMN_SAHPISENSORREADINGNORMALMINSTATUS:
	    /** INTEGER = ASN_INTEGER */
      snmp_set_var_typed_value (var, ASN_INTEGER,
				(char *) &context->
				saHpiSensorReadingNormalMinStatus,
				sizeof (context->
					saHpiSensorReadingNormalMinStatus));
      break;

    case COLUMN_SAHPISENSORREADINGNORMALMINEVENTSTATUS:
	    /** OCTETSTR = ASN_OCTET_STR */
      snmp_set_var_typed_value (var, ASN_OCTET_STR,
				(char *) &context->
				saHpiSensorReadingNormalMinEventStatus,
				context->
				saHpiSensorReadingNormalMinEventStatus_len);
      break;
