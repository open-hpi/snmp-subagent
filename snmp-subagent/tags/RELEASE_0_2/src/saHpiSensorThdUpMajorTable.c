...
oid saHpiSensorThdUpMajorTable_oid[] =
  { saHpiSensorThdUpMajorTable_TABLE_OID };
size_t saHpiSensorThdUpMajorTable_oid_len =
OID_LENGTH (saHpiSensorThdUpMajorTable_oid);
			   ...
			   SaHpiSensorNumT sensor_num)
{
  ...
  DEBUGMSGTL ((AGENT, "delete_ThdUpMajor_row (%d, %d, %d). Entry.\n",
  ...
  if (ctx)
    {
      ...
      saHpiSensorThdUpMajorTable_delete_row (ctx);
      rc = AGENT_ERR_NOERROR;
    }
  DEBUGMSGTL ((AGENT, "delete_ThdUpMajor_row. Exit (rc: %d).\n", rc));
  return rc;
}
			 ...
			 SaHpiSensorReadingT * reading)
{
  ...
  DEBUGMSGT ((AGENT, "\t--- populate_ThdUpMajor, Entry,\n"));
  ...
  if (reading && threshold_def)
	...
	{
	  snmp_log (LOG_ERR,
		    "Not enough memory for a sensor ThdUpMajor row!");
  ...
  DEBUGMSGT ((AGENT, "\t--- populate_ThdUpMajor, Exit (rc: %d),\n", rc));
					   ...
					   * ctx)
{
  ...
  DEBUGMSGTL ((AGENT, "Modify saHpiSensorThdUpMajorTable_ctx: Entry.\n"));
  ...
  if (ctx)
      ...
      if (ctx->hash != 0)
	{
	  ...
	  if ((ctx->domain_id == domain_id) &&
	      ...
	      (ctx->sensor_id == sensor_num))
	    {
	      ...
	      DEBUGMSGTL ((AGENT,
			   "Updating ThdUpMajorTable row [%d, %d, %d]\n",
			   domain_id, resource_id, sensor_num));
	    }
	}
	...
	hash = 1;		/* Need this - we consider hash
      ...
      build_reading_strings (reading,
			     0,
			     &ctx->saHpiSensorThdUpMajorValuesPresent,
  ...
  DEBUGMSGTL ((AGENT,
	       "Modify saHpiSensorThdUpMajorTable_ctx: Exit (NULL DATA)"));
...
int set_ThdUpMajor (saHpiSensorThdUpMajorTable_context *ctx) {
  ...
  DEBUGMSGTL ((AGENT, "set_ThdUpMajor: Entry.\n"));
  if (ctx)
    {
      ...
      if (rc != SA_OK)
	{
      ...
      build_reading_strings (&thd.UpMajor,
			     0,
			     &ctx->saHpiSensorThdUpMajorValuesPresent,
			     &ctx->saHpiSensorThdUpMajorRaw,
			     ctx->saHpiSensorThdUpMajorInterpreted,
 ...
 */
static int
  saHpiSensorThdUpMajorTable_row_copy
  (saHpiSensorThdUpMajorTable_context * dst,
 ...
 */
netsnmp_index *saHpiSensorThdUpMajorTable_delete_row
  (saHpiSensorThdUpMajorTable_context * ctx)
{
  /*
 ...
 */
void
saHpiSensorThdUpMajorTable_set_commit (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorThdUpMajorTable_set_free (netsnmp_request_group * rg)
{
 ...
 */
void
saHpiSensorThdUpMajorTable_set_undo (netsnmp_request_group * rg)
{
...
initialize_table_saHpiSensorThdUpMajorTable (void)
{
  ...
  if (my_handler)
    {
      snmp_log (LOG_ERR,
		"initialize_table_saHpiSensorThdUpMajorTable_handler called again\n");
      return;
    }
  ...
  my_handler =
    ...
    ("saHpiSensorThdUpMajorTable",
     ...
     saHpiSensorThdUpMajorTable_oid_len, HANDLER_CAN_RWRITE);
  ...
  cb.duplicate_row =
    (UserRowMethod *) saHpiSensorThdUpMajorTable_duplicate_row;
  cb.delete_row = (UserRowMethod *) saHpiSensorThdUpMajorTable_delete_row;
  cb.row_copy =
    (Netsnmp_User_Row_Operation *) saHpiSensorThdUpMajorTable_row_copy;

  cb.can_delete =
    (Netsnmp_User_Row_Action *) saHpiSensorThdUpMajorTable_can_delete;
