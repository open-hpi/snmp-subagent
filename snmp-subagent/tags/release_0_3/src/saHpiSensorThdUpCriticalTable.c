  ...
  if (reading && threshold_def)
      ...
      if (!ctx)
	{
	  /* 
	     New entry. Create it.
	   */
	  ctx = saHpiSensorThdUpCriticalTable_create_row (&sensor_thd_index);
	}
      if (!ctx)
	{
	  snmp_log (LOG_ERR,
  ...
  if (ctx)
      ...
      if (threshold_def)
	{
	  ctx->saHpiSensorThdUpCriticalIsReadable =
	    ((threshold_def->ReadThold & SAHPI_STM_UP_CRIT) ==
	     SAHPI_STM_UP_CRIT) ? MIB_TRUE : MIB_FALSE;

	  ctx->saHpiSensorThdUpCriticalIsWritable =
	    ((threshold_def->WriteThold & SAHPI_STM_UP_CRIT) ==
	     SAHPI_STM_UP_CRIT) ? MIB_TRUE : MIB_FALSE;
	  ctx->saHpiSensorThdUpCriticalIsFixed =
	    ((threshold_def->FixedThold & SAHPI_STM_UP_CRIT) ==
	     SAHPI_STM_UP_CRIT) ? MIB_TRUE : MIB_FALSE;
	}
      /* END */
...
int
set_ThdUpCritical (saHpiSensorThdUpCriticalTable_context * ctx)
{
  ...
  if (ctx)
	...
	{
	  thd.UpCritical.Interpreted.Type =
   ...
   saHpiSensorThdUpCriticalTable_context * src)
{
  ...
  dst->saHpiSensorThdUpCriticalIsFixed = src->saHpiSensorThdUpCriticalIsFixed;
...
saHpiSensorThdUpCriticalTable_set_reserve1 (netsnmp_request_group * rg)
{
  ...
  DEBUGMSGTL ((AGENT,
	       "saHpiSensorThdUpCriticalTable_set_reserve1. Entry.\n"));
  ...
  DEBUGMSGTL ((AGENT,
	       "saHpiSensorThdUpCriticalTable_set_reserve1. Exit. (rc:%d) \n",
	       rc));
}
...
saHpiSensorThdUpCriticalTable_set_reserve2 (netsnmp_request_group * rg)
{
  ...
  DEBUGMSGTL ((AGENT,
	       "saHpiSensorThdUpCriticalTable_set_reserve2. Entry.\n"));
  for (current = rg->list; current; current = current->next)
    {
  ...
  DEBUGMSGTL ((AGENT,
	       "saHpiSensorThdUpCriticalTable_set_reserve2. Exit (rc:%d).\n",
	       rc));
}
...
saHpiSensorThdUpCriticalTable_set_action (netsnmp_request_group * rg)
{
  ...
  for (current = rg->list; current; current = current->next)
	...
	case COLUMN_SAHPISENSORTHDUPCRITICALINTERPRETED:
	  ...
	  row_ctx->saHpiSensorThdUpCriticalInterpreted_len = var->val_len;
	  if (set_ThdUpCritical (row_ctx) != AGENT_ERR_NOERROR)
	    rc = SNMP_ERR_GENERR;
  ...
  DEBUGMSGTL ((AGENT,
	       "saHpiSensorThdUpCriticalTable_set_action. Exit (rc: %d)\n",
	       rc));
}
					 ...
					 * table_info)
{
  ...
  if (rc == AGENT_ERR_NOERROR)
    {
      modify_saHpiSensorThdUpCriticalTable_row (context->domain_id,
						context->resource_id,
						context->sensor_id,
						NULL,
						&sensor_threshold.
